## 1. 인증 및 kubeconfig
모든 쿠버네티스 클러스터를 사용하는 사용자는 kubectl 클라이언트나 HTTP REST 요청을 API 서버(kube-apiserver)에 접근한다. 해당 사용자는 제공한 인증 정보를 바탕으로 API 접근 권한을 부여받을 수 있다. 
...
kubeconfig 파일은 기본적으로 $HOME/.kube 디렉토리에 config 라는 파일이름으로 존재한다. 즉, kubectl 명령이 인증을 위해 참조하는 기본 kubeconfig 파일은 $HOME/.kube/config 파일이다. 만약 다른 kubeconfig 파일로 인증을 하고자 한다면 KUBECONFIG 쉘 환경 변수에 파일의 경로를 지정하거나, --kubeconfig 옵션을 사용하여 지정할 수 있다.
...
다시 돌아가서 기존에 사용하던 ```$HOME/.kube/config``` 파일을 확인해보자.
```
$ kubectl config view
