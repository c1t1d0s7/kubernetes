...
#### (2) 클라이언트 CSR 생성
생성한 클라이언트 개인키로 인증서 서명 요청(CSR: Certificate Signing Request)을 생성한다.
```
...
```
BASE64로 인코딩된 값은 CSR 리소스 생성 시 필요하기 때문에 클립보드 등 복사한다.
...
```
myuser3 CSR 리소스의 상태(CONDITION)은 승인되지 않은 상태이며, kube-apiserver-client 서명자는 자동 승인을 하지 않기 때문이다.
...
#### (2) 롤 바인딩 생성
myuser3 사용자와 admin-pod 롤을 연결해 준다.
> rbind-myuser3.yaml
```yaml
apiVersion: rbac.authorization.k8s.io/v1
kind: RoleBinding
...
myuser3@cluster.local 컨텍스트로 전환한다.
```
