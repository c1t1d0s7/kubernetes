...
현재 최신 버전은 ECK 1.4 이며, ECK 1.4 지원 버전은 다음과 같다.
- kubectl 1.11+
...
Elasticsearch에 접근하기 위한 서비스를 확인 해보자.
```
...
elastic 사용자와 패스워드 변수로 인증해 Elasticsearch가 응답하는지 확인 해보자.
```
...
```
Filebeats는 데몬셋으로 노드 개수만큼 생성되며 모든 상태가 green인지 확인하자.
...
Kibana 서비스의 EXTERNAL-IP를 확인 해보자.
```
...
```
IP는 192.168.56.200 이며, 포트는 5601 이다.
