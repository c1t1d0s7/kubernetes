...
TLS 종료 프록시(Termination Proxy)는 웹 서버(파드)에 직접 TLS 서비스를 구성하지 않고, 웹 클라이언트와 웹 프록시(로드밸런서)사이에서만 TLS(HTTPS)로 암호화 하고, 내부 네트워크의 TLS 종료 프록시와 웹 서버(파드) 사이는 일반 HTTP로 통신하는 방식을 구성한다.
