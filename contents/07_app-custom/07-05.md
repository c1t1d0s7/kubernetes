...
default-token-x4tb2 토큰 시크릿을 자세히 확인 해보자.
```
...
```
$ openssl req -new -x509 -key nginx-tls/nginx-tls.key -out nginx-tls/nginx-tls.crt -days 3650 -subj /CN=mynapp.example.com
```
...
#### (5) HTTPS 웹 서비스 확인
Nginx 파드에 HTTPS가 잘 적용됬는지 확인 해보자.
...
![TLS 종료 프록시](img/10_tls_termination_proxy.png)
...
```
$ openssl req -new -x509 ingress-tls/ingress-tls.key -out ingress-tls/ingress-tls.crt -days 3650 -subj /CN=mynapp.example.com
```
...
mynapp-ing-tls-term   mynapp.example.com   192.168.56.21,192.168.56.22,192.168.56.23   80, 443   13s
```
...
```
$ curl --resolve mynapp.example.com:443:192.168.56.21 -k -v https://mynapp.example.com
