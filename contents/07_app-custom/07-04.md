## 4. 컨피그 맵을 이용한 사용자화
컨피그 맵(ConfigMap)은 간단한 설정, 변수, 설정 파일의 값을 키/값 쌍 형태로 저장하고 사용할 수 있는 쿠버네티스 저장소다.

설정값을 앞서 살펴본 환경 변수나 명령의 인수로 제공할 수도 있고, 설정 파일(예: nginx.conf) 및 간단한 파일을 저장했다가 볼륨으로 제공할 수도 있다.
...
컨피그 맵에 사용할 파일과 디렉터리 및 데이터는 다음과 같다.
```
configmap/
└── message [Hello World by ConfigMap]
```

해당 파일을 이용하여 컨피그 맵을 생성해보자.
```
...
```
Nginx 설정 파일로 기본 설정에 gzip 압축 기능을 활성화하였다.
...
```
nginx-gzip-config 컨피그 맵을 볼륨으로 선언하고 /etc/nginx/conf.d 디렉터리에 마운트 한다.
...
...
```
볼륨이 마운트 되어 있으며, 볼륨은 nginx-compress-config 컨피그 맵에 의해 제공된 것을 확인할 수 있다.
...
새로운 터미널에서 curl 명령으로 확인해보자. -H 옵션은 HTTP 요청 헤더를 추가하는 부분이며, "Accept-Encodong: gzip"은 gzip 압축을 요청을 한다. -I 옵션은 HTTP 헤더만 보겠다는 의미이다.
``` 
...
```
응답 헤더에 "Content-Encoding: gzip" 항목이 보인다면 응답이 압축되었다는 의미다.
...
> 방금 생성한 파드의 볼륨 마운트 설정은, /etc/nginx/conf.d/ 디렉터리에 컨피그 맵 파일을 마운트 하게 된다.  
> 기존에 /etc/nginx/conf.d/ 디렉터리에 기존 파일이 있었다면, 기존 파일이 삭제되는 것은 아니지만 마운트 된 상태에서는 사용할 수 없다. 즉, 파일을 디렉터리 전체에 마운트 하지 말고, 개별 파일로 마운트 할 수 있다.
> ```
> spec:
>   containers:
>     volumeMounts:
